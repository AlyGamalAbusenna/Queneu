...
export default function App() {
  ...
  const handleNext = () => {
    ...
    // Stop current audio and clear timers if skipping
    if (audioRef.current) {
  ...
  return (
    ...
    >
          ...
          <AnimatePresence>
            {isAudioPlaying && !isTransitioning && (
              <motion.button
                ...
                exit={{ opacity: 0, x: 10 }}
                onClick={handleNext}
                className="px-4 py-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/10 rounded-full text-xs font-bold uppercase tracking-widest transition-colors flex items-center space-x-2"
              >
                <span>Skip</span>
      ...
      {/* Bottom Center: Next Button (Visible only when audio ends) */}
